...
class Reach(composer.Task):
    def __init__(self,
        ...
        self._task_observables = cameras.add_camera_observables(
            self._arena, cameras.FRONT_CLOSE, width=84, height=84
        )
    ...
    def initialize_episode(self, physics, random_state):
        ...
        # eq_data = np.zeros((11,))
        # eq_data[:3] = tcp.pos
        # eq_data[3:7] = np.array([0, 0, 0, 0])
        # weld.data = eq_data
        # weld.active = 1
        physics.forward()

        limits = physics.bind(self._arm.actuators).ctrlrange.T
        low, high = np.split(limits, 2)
        self._act_slope = (high - low) / 2
        self._act_intercept = (high + low) / 2

    def before_step(self, physics, action, random_state):
        ctrl = self._act_slope * action + self._act_intercept
        physics.bind(self._arm.actuators).ctrl = ctrl

    def action_spec(self, physics):
        lim = np.full((len(self._arm.joints),), 1., dtype=np.float32)
    ...
    #     #                 constants.DOWN_QUATERNION)
    #     physics.bind(self._arm.joints).qpos = pos
